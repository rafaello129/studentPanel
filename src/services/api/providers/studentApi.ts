...
const studentApi = peesadApi.injectEndpoints({
    ...
    endpoints: (builder) => ({
        ...
        getStudents: builder.query< ApiResponseAll<Student>, PaginationQueryParamsType >({

            query: ({ page = 1, limit = 1, isActive }) => ({
                url: `students/findAll?page=${page}&limit=${limit}` + (isActive !== undefined ? `&isActive=${isActive}` : ''),
                method: 'GET'
            }),
